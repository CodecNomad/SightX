...
int main(int, char **) {
  ...
  GLFWwindow *window = glfwCreateWindow(1920, 1080, "SightX", nullptr, nullptr);
  ...
  // Our state
  int selected_crosshair = 0;
  int crosshair_size = 3;
  int crosshair_gap = 1;
  bool render_crosshair = false;
  bool rainbow = false;
  ImVec4 crosshair_color = ImVec4(1.0f, 1.00f, 1.00f, 1.00f);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (rainbow) {
      ...
      crosshair_color = ImVec4(r, g, b, 1.0f);
    }

    if (ImGui::BeginMainMenuBar()) {
      if (ImGui::BeginMenu("Designer")) {
        static const char *items[]{"Dot", "Cross"};
        (ImGui::Combo("Shape", &selected_crosshair, items,
                      IM_ARRAYSIZE(items)));

        if (selected_crosshair == 1)
          ImGui::SliderInt("Gap", &crosshair_gap, 0, 150);
        ImGui::SliderInt("Size", &crosshair_size, 1, 150);
      ...
      if (ImGui::BeginMenu("Settings")) {
        ImGui::Checkbox("Render Crosshair", &render_crosshair);
        ImGui::EndMenu();
      }
    ...
    if (render_crosshair) {
      ...
      unsigned int converted_crosshair_color =
          ImGui::ColorConvertFloat4ToU32(crosshair_color);
      switch (selected_crosshair) {
      case 0: {
        draw_list->AddCircleFilled(center, crosshair_size,
      ...
      case 1: {
        ...
        draw_list->AddLine({cx - crosshair_gap - crosshair_size, cy},
                           {cx - crosshair_gap, cy}, col);

        draw_list->AddLine({cx + crosshair_gap, cy},
                           {cx + crosshair_gap + crosshair_size, cy}, col);

        draw_list->AddLine({cx, cy - crosshair_gap - crosshair_size},
                           {cx, cy - crosshair_gap}, col);

        draw_list->AddLine({cx, cy + crosshair_gap},
                           {cx, cy + crosshair_gap + crosshair_size}, col);
        break;
      }
      }
    }
